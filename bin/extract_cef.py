...
def main(args):
    ...
    try:
        fh = open(args.cef_table,"r")
        ip_re = re.compile(r"\d+\.\d+\.\d+\.\d+")
        ip_addr = next_hop = intf = match_ip_re = ok_to_process = is_ignore \
        = None
        # the two interface below goes to the core instead
        EXCLUDE_INTFS = ('tengigabitethernet7/3','tengigabitethernet7/5')
        # the browse interface to check
        BROWSE_INTFS = ('vlan370')
        # the browse ASA next hop to check
        EXCLUDE_NH = ('192.168.45.5')
        ...
        for line in fh:
            ...
            if len(elem) == 3:
                ...
                continue
            match_ip_re = ip_re.match(next_hop)
            if not match_ip_re:
                continue
            if not IPv4.isRFC1918(ip_addr):
                if intf.lower() not in EXCLUDE_INTFS:
                    if intf.lower() in BROWSE_INTFS:
                        if next_hop not in EXCLUDE_NH:
                            ok_to_process = False   # no need to check for multiple next hop
                            vpn_prefix.append(ip_addr)
                            continue
                    else:
                        ok_to_process = False   # no need to check for multiple next hop
                        if intf.lower() == 'vlan350':
                            extranet_prefix.append(ip_addr)
                        elif intf.lower() == 'vlan603' or intf.lower() == 'vlan604':
                            international_prefix.append(ip_addr)
                        else:
                            vpn_prefix.append(ip_addr)
                        continue
    ...
    uniqueness = {} 
    out.writeln("!!! bypass international MPLS destination")
    for i in international_prefix:
        if i not in uniqueness:
            uniqueness[i] = 1
            out.writeln(i)
        else:
            out.writeln("!!! " + i)
    out.writeln("!!! bypass Extranet destination")
    for i in extranet_prefix:
        if i not in uniqueness:
            uniqueness[i] = 1
            out.writeln(i)
        else:
            out.writeln("!!! " + i)
    out.writeln("!!! bypass VPN destination")
    for i in vpn_prefix:
        if i not in uniqueness:
            uniqueness[i] = 1
            out.writeln(i)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Parse the MetLife CEF table to filter out public IP \
that's NOT using the Internet Browse environment",
        epilog = "Happy coding!",
        prefix_chars='-'
        )
    #add position mandatory argument
    ap.add_argument('cef_table', help="ip cef table from the egress router")
